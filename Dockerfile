...
RUN cargo build --release --bin provider --bin test --no-default-features --locked
...
COPY --from=provider-builder /usr/src/provider/target/release/test /usr/local/bin/test
ENTRYPOINT ["test"]